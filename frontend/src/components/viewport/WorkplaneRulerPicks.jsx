...
function PickMeasurement({ origin, pick, unitSystem, onRemove }) {
  ...
  return (
    <group>
      ...
      {/* Chips cluster — fixed screen-space offset above-right of the
          pick point so it never sits on top of geometry. The 3D
          dashed leader line above (origin → pick point) doubles as
          the visual connector; an extra HTML connector caused stale
          layouts on first render (iter-114.8 bugfix). */}
