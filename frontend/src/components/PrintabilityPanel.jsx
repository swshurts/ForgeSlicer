...
export default function PrintabilityPanel() {
    ...
    // Re-run checks whenever the scene's object list changes shape (add /
    // remove / replace) or the build volume changes. Cheap enough that
    // we don't debounce for v1; revisit if the user reports lag.
    useEffect(() => {
                ...
                {findings.length === 0 ? (
                    <div className="px-6 py-10 text-center" data-testid="printability-empty-state">
                        ...
                        <p className="mt-2 text-[11px] text-slate-500 leading-relaxed">
                            Manifold geometry · within build volume · no floating parts detected.
                            Future checks (thin walls · overhangs · small features) will be added here.
                        </p>
                    </div>
                ) : (
