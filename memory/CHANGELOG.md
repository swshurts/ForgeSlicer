...
## Iteration 125.1 (2026-07-04) — Ruler regression fixes (stacked objects)
User report: on preview, after placing a cube + cone (cone on top), placing the ruler at the front-left corner and trying to reposition to measure to the tip of the cone gave off readings. Symptoms: (b) reposition doesn't work, (c) ruler doesn't stick to picked corner, (e) distance chips missing, (g) unresponsive on stacked objects.

Root causes identified & fixed:
1. **`priorityRaycast` tie-break bug** (real regression) — All placement dots were setting `hit.distance = -1e-4`, a hard tie. When two dots overlapped in screen space (cube-top-corner + cone-bottom-corner share the exact world coord; or top vs bottom bbox corners project to the same pixel), THREE's stable-sort left the winner up to insertion order — often the dot BEHIND the visible geometry won. Fix: `hit.distance = -1 + hit.distance * 1e-6` — all priority hits stay negative (< -0.999, still beating regular geometry which has positive distance), but among priority hits the closer-to-camera dot now wins.
2. **Missing apex/top-center placement dots** — dots only covered the 8 bbox corners. Users couldn't pick the tip of a cone or top of a cylinder. Fix: added `top-center` + `bottom-center` synthetic dots per object bbox (cone tip = top-center of bbox).
3. **Elevation invisible in origin label** — ruler placed at (10, -10, 20) rendered exactly like a bed-level placement in the label. Fix: when `|oz| > 0.05mm`, append an amber `↑ 20mm` tag (`data-testid="workplane-ruler-origin-z"`) so users immediately see the ruler is on top of a stacked object — clarifies why "cone at top of cube" chips read (0, 0, 20) instead of (0, 0, 44).
