...
- **Beginner CAD toolset expansion** (filed 2026-06-27 from Steve's stakeholder review). The goal is to lift the modelling surface to the level a TinkerCAD user would expect at first sight. Every tool below must ship beginner-friendly: clear lucide icon, one-line description, numeric inputs in mm, one contextual help example ("e.g. fillet a 2 mm corner for a soft edge"), and a sensible default that "just works" if the user touches nothing.
    ...
    - **Align / Distribute** — ~~multi-select two-plus objects → align~~ [DONE iter-65 for Align, iter-110 for Distribute]. The Align popover now has both 9 alignment cells (X/Y/Z × min/centre/max) and a Distribute row (3 axis buttons, ≥ 3 objects required).
    - **Measurement / ruler tools** — ~~snap-to-vertex / edge-midpoint / face-centre, persistent measurement labels~~ [DONE iter-105/110 via Anchor Ruler + pinnedRulerDims], ~~unit toggle (mm / inch / cm)~~ [DONE iter-112 — mm/in global toggle in StatusBar]. ~~Inline-editable W/D/H labels on selection + TinkerCAD-style draggable Workplane Ruler~~ [DONE iter-113].
    ...
    - **Snap-to-face placement** — ~~drop or paste an object → it auto-orients to the nearest face under the cursor (face normal becomes the object's +Z)~~ [DONE iter-113 — toolbar `place-on-face-toggle-btn`, single-shot teleport via `lib/placeOnFace.js`]. Future: a clip-to-face-bbox toggle and drag-time live preview.
    - **Reusable component library** — ~~a third LeftPanel tab ("Library")~~ [DONE iter-110 — new "Lib" tab with 8 parametric component recipes (M3 standoff, 608 bearing seat, GoPro mount, L bracket, cable clip, pin hinge, spur gear, knob), filterable by category]. Future: expand the catalogue (M-series screws + nuts, hinges piano/butt, S/J hooks, USB-A/C/3.5 mm jack cutouts, DIN-rail clip), surface a search box, allow users to save their own components into the library.
...
- **Pre-flight Printability Checks** (filed 2026-06-27 from Steve's stakeholder review). Catch the failures that turn a new printer-owner's first weekend into a frustration spiral. Runs automatically on the active scene before any export / slice, and is also accessible on-demand via a toolbar button ("Check my print"). Surfaces a single dismissible side panel with one row per finding — plain-language headline + severity pill + visual highlight + one-click fix button. Beginner-friendly framing first; technical detail collapsed under "Show details" for the engineers.
    - **Checks to implement** (priority order — start with the ones that account for ~80 % of failed first prints):
        1. ~~**Non-manifold / open geometry**~~ [DONE — iter 108] — find holes, T-junctions, inverted normals, disconnected shells. Headline: *"Your model has gaps the slicer can't seal."* Fix button: **Repair mesh** (calls the existing watertight-heal pipeline already used during slicer handoff).
        2. ~~**Thin walls**~~ [DONE — iter 109] — async raycast / medial-axis scan via `three-mesh-bvh`, sampling ≤ 600 tris per object. Flags walls < 0.8 mm. Fix: **silence** (educational — wall thickness is best fixed in the source editor).
        3. ~~**Overhangs**~~ [DONE — iter 109] — face-normal vs +Z, 45° hard cutoff, > 5 % triangle area flags. Fix: **Auto-orient flat side down** (rotates so the dominant flat face touches Z=0).
        4. ~~**Floating parts**~~ [DONE — iter 109] — object world bbox.min.z > 0.1 mm flags. Fix: **Drop to bed** (wraps the existing `scene.dropToBed`).
        5. ~~**Intersecting geometry**~~ [DONE — iter 109] — pairwise AABB overlap of positives. Fix: **Select both to union** (sets `selectedIds` to the offending pair; user then clicks Union (∪)).
        6. ~~**Build-volume violations**~~ [DONE — iter 109] — world bbox vs `scene.buildVolume`. Fix: **Scale scene to fit** (wraps `scene.resizeSceneToBed`).
        7. ~~**Very small features**~~ [DONE — iter 109] — bbox shortest dim < 0.6 mm flags. Fix: **Scale up to 1 mm minimum** (uniform scale so shortest dim hits SAFE_PRINT_MIN_MM=1.0).
